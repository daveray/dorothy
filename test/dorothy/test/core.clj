...
(deftest test-node-id
  (testing "returns ::type ::node-id"
    (is (= { ::d/type ::d/node-id :id :foo :port :bar :compass-pt :n}
           (d/node-id :foo :bar :n))))
...
(deftest test-node
  ...
  (testing "return ::type ::node"
    (is (= {::d/type ::d/node :attrs {:a 1} :id (d/node-id :foo) }
...
(deftest test-edge
  ...
  (testing "return ::type ::edge"
    (is (= {::d/type ::d/edge :attrs {:a 1} :node-ids [(d/node-id :foo)(d/node-id :bar)] }
           (d/edge {:a 1} [(d/node-id :foo)(d/node-id :bar)])))))

(deftest test-graph*
  (testing "returns ::type ::graph"
    (is (= {::d/type ::d/graph :id :G :strict? true :statements [] }
           (d/graph* {:id :G :strict? true} [])))))

(deftest test-digraph*
  (testing "returns ::type :::digraph"
    (is (= {::d/type ::d/digraph :id :G :strict? false :statements [] }
           (d/digraph* {:id :G } [])))))
